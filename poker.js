...
let talia = {
    ...
    FiveCardsCopy:  [ ],
    FiveCards:  [ ]
          //    { Id: 13, Color: "spades", Karta: '\u{1F0A6}'},
          //    { Id: 9, Color: "spades", Karta: '\u{1F0A7}'},
          //    { Id: 12, Color: "spades", Karta: '\u{1F0A8}' },
          //    { Id: 11, Color: "spades", Karta: '\u{1F0A9}' },
          //    { Id: 10, Color: "spades", Karta: '\u{1F0A9}' }
          // ]
           ,
  ...
  TakeRandom: function(count) {
    this.FiveCards.pop;
     for (let index = 0; index < count; index++) {
          let aa = Math.floor(Math.random() * (22-index)) ;//+ 1;
...
CheckIfStraightFlush : function(count) { //a,k,q,j,10 w kolorze
    ...
    for (let index = 0; index < this.FiveCards.length; index++) {
        if (element===this.FiveCards[index].Id) 
            { target[counter]=index; 
                counter++; 
         //       console.log('cel wystapil:',counter);//target.length,'targ1',target[index])
            };
      }
    if (counter === ile){
      ...
      for (let index = target.length; index >0;index--)
       ...
       //   console.log(counter,target);   console.log(this.FiveCards)
     }
    return counter;
 ...
 CheckIfIsStrit : function(count) {//1
  let bool=false;
  let max=0;
  let aceInArray=false;
  
     for(let i=0;i<this.FiveCards.length;i++){
      if (this.FiveCards[i].Id===1) {aceInArray=true;}
        if (this.FiveCards[i].Id>max)   { //znajdz max karte a potem kolejne mniejsze o jeden
              max=this.FiveCards[i].Id;
      ...
      if (aceInArray===true) {max=1;}            //  jesli as co ma index 1 to  strit to 1,13,12,11,10
      if (this.CheckIfIsFigura(max)>=0){             //  console.log('tumax',max)
         if (aceInArray===true) {max=14;} //jak as to potem sprawdz 13 12...
         if (this.CheckIfIsFigura(max-1)>=0){  //       console.log(max-1)
            if (this.CheckIfIsFigura(max-2)>=0){ //      console.log(max-2)
               if (this.CheckIfIsFigura(max-3)>=0){ //  console.log(max-3)
                  if (this.CheckIfIsFigura(max-4)>=0){ //console.log('last',max-4);
   ...
   return bool;  //jesli strit to 1 jesli nie -1 
  },//czy strit - kolejne nie w kolorze

  CheckIfISThree : function(count) {//1
    let bool=false;
    if (this.HowManyTimes(this.FiveCards[0].Id,3)===3)
       {  bool=true} //mamy trójkę
     else if (this.HowManyTimes(this.FiveCards[1].Id,3)===3)
       {  bool=true} //mamy trójkę
     else if (this.HowManyTimes(this.FiveCards[2].Id,3)===3)
       {  bool=true} //mamy trójkę
    ...
    CheckIfISTwoPairsOrOnePair : function(count) {//1
      let numPairs=0;
      //let bool2=false;
      if (this.HowManyTimes(this.FiveCards[0].Id,2)===2)
         {  numPairs=1;} //mamy 1 pare
       else if (this.HowManyTimes(this.FiveCards[1].Id,2)===2)
         {  numPairs=1;} //mamy 1 pare
       else if (this.HowManyTimes(this.FiveCards[2].Id,2)===2)
         {  numPairs=1} //mamy 1 pare
       else if (this.HowManyTimes(this.FiveCards[3].Id,2)===2)
         {  numPairs=1} //mamy 1 pare
    
   //   console.log('to zostalo',this.FiveCards);
      if (numPairs===1){
      //  console.log('num1',numPairs,'tab0=',this.FiveCards[0].Id,this.FiveCards[1].Id,this.FiveCards[2].Id);
        if (this.HowManyTimes(this.FiveCards[0].Id,2)===2)
        {  numPairs=2} //mamy 2 pare
         else if (this.HowManyTimes(this.FiveCards[1].Id,2)===2)
          {  numPairs=2} //mamy 2 pare
       }
     //  console.log('nummars',numPairs);
        return numPairs;
    },//czy full 3 i 2  

    CheckHighCard: function(count) {
      let max=0;
      let placeInArray=0;
    //  let aceInArray=false;
      let bool=false;
      for(let i=0;i<this.FiveCards.length;i++){
        if (this.FiveCards[i].Id===1) {
         // aceInArray=true;
          placeInArray=i;
         }
        else if (this.FiveCards[i].Id>max)   { //znajdz max karte a potem kolejne mniejsze o jeden
              max=this.FiveCards[i].Id;
              placeInArray=i;
          }
      }
...
console.log(" ");
//console.log("ROYAL FLUSH ??")
if (talia.CheckIfRoyalFlush()===true) 
  {console.log("ROYAL FLUSH")}; //dziala duzy poker
talia.FiveCards=talia.FiveCardsCopy.slice(); //console.log("STRAIGHT FLUSH??")
if (talia.CheckIfStraightFlush()===true)
  {console.log("STRAIGHT FLUSH")}; // maly poker
talia.FiveCards=talia.FiveCardsCopy.slice(); //console.log("Four??")
if (talia.CheckIfISFour()===true) 
  {console.log("Four of a kind")}; //kareta
talia.FiveCards=talia.FiveCardsCopy.slice(); //console.log("Full ??")
if (talia.CheckIfISFull()===true) 
  {console.log("Full house")}; //full
talia.FiveCards=talia.FiveCardsCopy.slice(); //console.log("Flush (kolor) ??")
if (talia.CheckIfSameColor()===true) 
  {console.log("FLUSH - kolor")}; // kolor
talia.FiveCards=talia.FiveCardsCopy.slice(); //console.log("Strit??");
 if (talia.CheckIfIsStrit()===true) 
  {console.log("Strit")}; //strit
talia.FiveCards=talia.FiveCardsCopy.slice(); //console.log("Three??");//trojka
if (talia.CheckIfISThree()===true) 
  {console.log("Three")}; //strit
talia.FiveCards=talia.FiveCardsCopy.slice();
//dwie pary
//console.log("Two or one pair??");
let aa=talia.CheckIfISTwoPairsOrOnePair();
if (aa===2) {    console.log("Two Pairs");     }//dwie pary
   else if(aa===1) {console.log("One Pair")     } //para     
talia.FiveCards=talia.FiveCardsCopy.slice();

//console.log("High Card??");
let aaa=talia.CheckHighCard();//w aaa pozycja najwyszej karty 
console.log(talia.FiveCards[aaa].Id,talia.FiveCards[aaa].Color,talia.FiveCards[aaa].Karta, 'Highest card'); //high card
talia.FiveCards=talia.FiveCardsCopy.slice(); //najwyzszakarta




//module.exports = {talia};